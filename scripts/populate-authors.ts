...
async function populateAuthors() {
  console.log('Starting author data population...');
  ...
  // Check current state
  const currentAuthors = await authorRepository.find({ order: { id: 'ASC' } });
  console.log(`Found ${currentAuthors.length} existing authors`);

  if (currentAuthors.length > 0) {
    console.log('Existing authors:');
    currentAuthors.forEach(author => {
      console.log(`  ${author.id}: "${author.slug}" -> name: "${author.name}", img: "${author.img}"`);
    });

    // Check if we need to update missing data
    const authorsWithoutNames = currentAuthors.filter(a => !a.name || a.name.trim() === '');
    const authorsWithoutImages = currentAuthors.filter(a => !a.img || a.img.trim() === '');

    console.log(`Authors without names: ${authorsWithoutNames.length}`);
    console.log(`Authors without images: ${authorsWithoutImages.length}`);

    if (authorsWithoutNames.length === 0 && authorsWithoutImages.length === 0) {
      console.log('All authors have complete data. Nothing to update.');
      return;
    }
  }

  // Clear existing data and insert fresh from migration
  console.log('Clearing existing data...');
  await AppDataSource.getRepository(AnalysisSchema).clear();
  await authorRepository.clear();

  console.log('Inserting fresh author data from migration...');

  // Insert the real author data from the migration
  await AppDataSource.query(`
    INSERT INTO \`Author\` (\`id\`, \`slug\`, \`name\`, \`img\`, \`bio\`) VALUES
    ...
    (3,'kochman','Adw. Oskar Kochman','/images/Kochman.png','Absolwent Wydziału Prawa i Administracji Uniwersytetu Warszawskiego. Adwokat – członek Izby Adwokackiej w Warszawie. Zawodowo od 5 lat związany z sektorem administracji publicznej. Zdobywał doświadczenie również jako prawnik w podmiotach gospodarczych i kancelariach prawnych. Autor analiz i publikacji prawnych i ekonomicznych. Działacz społeczny realizujący od kilku lat szereg projektów w trzecim sektorze. Główny obszar badawczy: rynek finansowy w wymiarze prawnym i ekonomicznym; analiza skutków regulacji; administracja publiczna; badania i analiza postaw społecznych i politycznych.\"'),
    (4,'rosolowski','Marcin Rosołowski','/images/Rosołowski.png','Absolwent Wydziału Prawa i Administracji Uniwersytetu Warszawskiego; w latach 2006-2008 zastępca dyrektora Biura Prasowego Kancelarii Prezydenta RP  współautor Pocztu przedsiębiorców polskich. Wiceprezes Fundacji im. XBW Ignacego Krasickiego; członek Rady Fundacji Instytut Staszica.\"'),
  ...
  console.log('Inserted analyses data');

  // Show final state
  const finalAuthors = await authorRepository.find({ order: { id: 'ASC' } });
  console.log('\nFinal state - first 5 authors:');
  finalAuthors.slice(0, 5).forEach(author => {
    console.log(`  ${author.id}: "${author.slug}" -> "${author.name}" (${author.img})`);
  });

  console.log(`\n✅ Data population completed! ${finalAuthors.length} authors with real data.`);
}
