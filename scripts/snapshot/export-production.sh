...
dump_database() {
  docker run --rm \
    ...
    --env-file "$mysql_env_file" \
    "$mysql_image" \
    sh -ec 'MYSQL_PWD="$MYSQL_PWD" exec mysqldump --host="$SOURCE_MYSQL_SERVICE" --user="$SNAPSHOT_EXPORT_USER" --single-transaction --quick --hex-blob --routines --triggers --events --skip-lock-tables --set-gtid-purged=OFF --no-tablespaces "$SOURCE_DATABASE"'
}
