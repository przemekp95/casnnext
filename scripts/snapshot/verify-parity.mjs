...
function databaseDump(container) {
  return command("docker", [
    "exec", container,
    "sh", "-ec",
    'MYSQL_PWD="$MYSQL_ROOT_PASSWORD" exec mysqldump --user=root --single-transaction --quick --hex-blob --routines --triggers --events --skip-lock-tables --set-gtid-purged=OFF --no-tablespaces --skip-dump-date --compact casn_local',
  ], { binary: true });
}
