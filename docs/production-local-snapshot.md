...
- Manifest zapisuje dla każdego rodzaju mediów reprezentatywną ścieżkę i źródło dowodu (`public-api`, `volume-inventory`, `empty-volume` lub jawne `no-public-directus-reference`). Prywatny rekord `directus_files` nie jest uznawany za publiczny asset. Każda zapisana ścieżka przechodzi później rzeczywisty test HTTP przez lokalny nginx.
