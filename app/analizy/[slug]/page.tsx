...
// Generuj metadane SEO dla każdej analizy
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  try {
    ...
    if (!analysis) {
      return {
        title: "Analiza nie znaleziona - Centrum Analiz Służby Niepodległej",
        description: "Przepraszamy, poszukiwana analiza nie została znaleziona.",
      };
    }

    const title = `${analysis.title} - Centrum Analiz Służby Niepodległej`;
    const description = analysis.author?.name
      ? `Analiza autorstwa ${analysis.author.name} w Centrum Analiz Służby Niepodległej. Przeczytaj najnowsze badania i raporty polityczne.`
      : `Analiza polityczna i społeczna w Centrum Analiz Służby Niepodległej. Przeczytaj najnowsze badania i raporty.`;

    return {
      title,
      description,
      keywords: ["analiza polityczna", "raport", "badania", "suwerenność", "niepodległość", analysis.author?.name].filter(Boolean),
      authors: [{ name: analysis.author?.name || "Centrum Analiz Służby Niepodległej" }],
      openGraph: {
        title,
        description,
        type: "article",
        authors: [analysis.author?.name || "Centrum Analiz Służby Niepodległej"],
      ...
      twitter: {
        ...
        title,
        description,
        creator: analysis.author?.name ? `@${analysis.author.name.replace(/\s+/g, '')}` : "@CASN_pl",
      },
    };
  } catch (error) {
    console.warn('generateMetadata failed:', error);
    return {
      title: "Analiza - Centrum Analiz Służby Niepodległej",
      description: "Analiza polityczna i społeczna w Centrum Analiz Służby Niepodległej.",
    };
  }
}
