...
export class InitialSetup1736424470000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    // Check if Author table exists
    const authorTableExists = await queryRunner.query(`
      SELECT TABLE_NAME
    ...
    const analysisTableExists = await queryRunner.query(`
      SELECT TABLE_NAME
      FROM INFORMATION_SCHEMA.TABLES
      WHERE TABLE_SCHEMA = DATABASE()
    ...
    // Check if Author table is empty before inserting data
    if (authorTableExists.length > 0) {
      const authorCount = await queryRunner.query(`
        SELECT COUNT(*) as count FROM \`Author\`
      `);

      if (authorCount[0].count === 0) {
        // Insert Author data only if table exists but is empty
        await queryRunner.query(`
          ...
          (6,'domanska','Dr Aldona Domańska','Dr Aldona Domańska','/images/Domanska.png','Prezes Instytutu Staszica; adiunkt Instytutu Studiów Międzynarodowych Szkoły Głównej Handlowej w Warszawie; doktor habilitowany nauk ekonomicznych. Zainteresowania badawcze: Makroekonomia gospodarki otwartej; w szczególności zagadnienia polityki gospodarczej – jej skuteczności i uwarunkowań w gospodarkach otwartych; znaczenie międzynarodowych współzależności dla efektywności polityki fiskalnej państwa; polityka gospodarcza a udział kraju w międzynarodowym ugrupowaniach integracyjnych; rozprzestrzenianie się kryzysów regionalnych i globalnych.\"'),
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    // Check if Analysis table exists before dropping foreign key
    const analysisTableExists = await queryRunner.query(`
      SELECT TABLE_NAME
      FROM INFORMATION_SCHEMA.TABLES
      WHERE TABLE_SCHEMA = DATABASE()
    ...
    // Check if Author table exists before dropping
    const authorTableExists = await queryRunner.query(`
      SELECT TABLE_NAME
      FROM INFORMATION_SCHEMA.TABLES
      WHERE TABLE_SCHEMA = DATABASE()
